                            ...
                            { columns.map ( item => (
                                item.sortable === undefined || item.sortable === true ? 
                                    <th scope="col" onClick={() => { handleTableHeadFilter(item.dataTitle) }}>
                                        <span 
                                            className={`d-block ${tableHeadFilter.column === item.dataTitle ? "dropdown-toggle" :""} ${tableHeadFilter.ascending === false ? "return" : ""}`}>{item.title}</span>
                                    </th>
                                ...
                                datasToShow.length > 0 ? datasToShow.map((item, index) => (
                                    <tr key={index}>
                                        ...
                                        { columns.map ((colItem, index) => (
                                            ...
                                            <td>{colItem.renderData(item, index = null)}</td> : (
                                            index === 0 ? <th scope="col">{ item[colItem.dataTitle] }</th> : <td>{ item[colItem.dataTitle] }</td> )
                                        ))}
                                    </tr>
                                )):
                                <tr>
